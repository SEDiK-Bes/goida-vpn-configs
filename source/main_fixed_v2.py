# МОДИФИЦИРОВАННАЯ ВЕРСИЯ main.py С КРИТИЧЕСКИМИ ИСПРАВЛЕНИЯМИ
# Версия 2.0: Исправлены все выявленные уязвимости и проблемы безопасности
# Автор модификации: Критический анализ и исправления
...
PING_WORKERS = 20
MAX_PORT = 65535  # ИСПРАВЛЕНИЕ #2: Максимальный порт TCP/UDP
MIN_PORT = 1      # Минимальный порт

# ================== ЛОГИРОВАНИЕ И СИНХРОНИЗАЦИЯ ==================
LOGS_BY_FILE: dict[int, list[str]] = defaultdict(list)
_LOG_LOCK = threading.Lock()
_UPDATED_FILES_LOCK = threading.Lock()
_README_UPDATE_LOCK = threading.Lock()  # ИСПРАВЛЕНИЕ #4: Lock для README

# Настройка logging для лучшей отладки
...
# ИСПРАВЛЕНИЕ #1: Удаление дублирования REPO_NAME
def get_repo_name():
    """Получение имени репозитория с приоритизацией переменной окружения"""
    repo_name = os.environ.get("REPO_NAME", "SEDiK-Bes/goida-vpn-configs")
    log(f"ℹ️ Используется репозиторий: {repo_name}")
    return repo_name
...
try:
    ...
    log(f"✅ Успешное подключение к репозиторию {REPO_NAME}")
except Exception as e:
    log(f"❌ Ошибка подключения к репозиторию: {e}")
    raise
...
def extract_host_port_from_config(config_line: str) -> tuple:
    """Извлекает хост и порт из конфигурационной строки с валидацией"""
    try:
        ...
        if config_line.startswith('vmess://'):
            try:
                ...
                # ИСПРАВЛЕНИЕ #3: Безопасная обработка JSON
                try:
                    decoded = base64.b64decode(payload).decode('utf-8', errors='ignore')
                    if decoded.startswith('{'):
                        j = json.loads(decoded, strict=False)
                        host = j.get('add') or j.get('host') or j.get('ip')
                        port = j.get('port')
                        
                        # Валидация порта
                        if host and port and validate_port(int(port)):
                            return str(host), int(port)
                except json.JSONDecodeError as je:
                    logger.debug(f"JSON decode error: {je}")
                    return None, None
                except (ValueError, TypeError) as ve:
                    logger.debug(f"Value/Type error: {ve}")
                    return None, None
            except Exception as e:
                logger.debug(f"VMess parsing error: {e}")
                pass
        ...
        if match:
            ...
            try:
                ...
                if validate_port(port_int):
                    return match.group(1), port_int
            except (ValueError, TypeError):
                return None, None

    except Exception as e:
        logger.debug(f"extract_host_port error: {e}")
        pass
...
def check_server_availability(host: str, port: int, timeout: float = CONNECTION_TIMEOUT) -> int:
    ...
    try:
        ...
        logger.debug(f"Ping check error: {e}")
        return -1
...
def test_single_config(config_line: str) -> tuple:
    ...
    if not host or not port or not validate_port(port):
...
def filter_and_sort_best_configs(configs: list, local_path: str) -> list:
    """Фильтрует и сортирует конфиги по доступности"""
...
# ================== ИСПРАВЛЕНИЕ #4: THREAD-SAFE README UPDATE ==================
def update_readme_table():
    """Обновляет таблицу в README.md с защитой от race conditions"""
    with _README_UPDATE_LOCK:  # Критическая секция
        try:
            try:
